...
For =GET /get-all-todos=:
- =q= optionally filters todos by a case-insensitive substring of =title=, =tags=, =todo= state, =category=, or =effectiveCategory=. Exact title matches sort before title-prefix matches, followed by other matches in their existing order.
- =limit= optionally caps the returned =todos= after filtering and ranking; it must be a positive integer.
- When either =q= or =limit= is supplied, =total= is the number of matching todos before =limit= is applied.
...
=POST /complete= accepts an optional JSON boolean =strict= (default =false=). In strict mode, an =id= must resolve exactly; otherwise =file=, =pos=, and =title= must identify the exact heading, with no title fallback. Conflicts return HTTP 409 without changing an item. =POST /update= and the =/set-state= alias support the same strict lookup behavior.
...
Enabled when =org-agenda-api-memory-file= (or =ORG_MEMORY_FILE=) names an org
file. Each top-level heading is a note: the title is its name and the body is
its text. Notes are hand-editable; the API keeps =ID=, =CREATED=, =UPDATED= and
an optional =SOURCE= in the property drawer.

| Endpoint              | Description                                                        |
|-----------------------+--------------------------------------------------------------------|
| =GET /memory=         | Search notes. =q= matches every whitespace-separated term, case-insensitively, against name, text and tags; name matches rank first. =offset=, =limit= (default 20, max 100) page results |
| =POST /memory/save=   | ={name, text, source?}= keeps a note, replacing one with the same name |
| =POST /memory/learn=  | Same body; adds the note tagged =:unreviewed:=. Returns 409 if a reviewed note has that name |
| =POST /memory/forget= | ={name}= deletes the note; 404 if none has that exact name        |

Saving over an unreviewed note, or removing its tag by hand, marks it reviewed.
Validation failures return 400 and change nothing.
...
| =ORG_MEMORY_FILE=      | Org file backing the /memory endpoints (disabled when unset) |
