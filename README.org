...
The =org-agenda-mcp= sidecar exposes four stdio MCP tools over the existing
HTTP API: =org_agenda=, =org_capture=, =org_complete=, and =org_update=. It does
not read or modify org files directly.
...
The Nix container includes =/bin/org-agenda-mcp=. Because stdio MCP servers are
child processes of their MCP host, supervisord does not start it as a daemon;
the existing supervisor configuration continues to manage only Emacs, nginx,
git sync, and health checking. A host that shares the container can launch it
on demand (for example, with =docker exec -i=) and point it at the container's
HTTP listener.
