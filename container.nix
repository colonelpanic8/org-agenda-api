  ...
  nginxConf = pkgs.writeText "nginx.conf" ''
    ...
    http {
      ...
      server {
        ...
        location ~ ^/(api/)?(agenda|agenda-files|get-all-todos|get-todays-agenda|complete|set-state|update|delete|delete-logbook-entry|reload|todo-states|todo-states-by-file|capture-templates|capture|custom-views|custom-view|debug-config|filter-options|metadata|restart|category-types|categories|category-tasks|category-capture|habit-config|habit-status|all-habit-statuses|notifications|trigger-sync|call-function|meta-score|meta-score-series|app-config(/[A-Za-z0-9_-]+)?|memory(/(save|learn|forget))?|add-clock|update-clock)$ {
